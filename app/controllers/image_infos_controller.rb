...
class ImageInfosController < ApplicationController
  before_action :set_imageinfo, only: %i[show edit update destroy]

  def index
    @image_infos = params[:sub_category_id] ? filter_image : list_image
  end
  ...
  def update
    ...
    @image_info.image.attach(params[:user][:image])
  ...
  def filter_image
    ImageInfo.where(sub_category_id: params[:sub_category_id])
  end
  ...
  def imageinfo_params
    params.require(:image_info)
          .permit(:img_name, :img_type, :description,
                  :price, :sub_category_id, :image)
  end
end
